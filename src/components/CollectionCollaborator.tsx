...
const CollectionCollaborator: React.FC<Props> = ({
  ...
  const api = useApi();
  const [collaborationId, setCollaborationId] = React.useState<number | null>(
    null
  );
  ...
  const deleteCollaborator = async () => {
    if (!is_collection_owner) return;
    setDeleteLoading(true);
    try {
      await api.delete(`/collections-collaborators/${collaborationId}`);
  ...
  return (
    <div className={``}>
      ...
      <Modal show={!!collaborationId} onClose={() => setCollaborationId(null)}>
        <Modal.Header>
        ...
        <Modal.Footer>
          ...
          <ButtonSecondary onClick={() => setCollaborationId(null)}>
            {t("Cancel")}
          </ButtonSecondary>
        </Modal.Footer>
      </Modal>
